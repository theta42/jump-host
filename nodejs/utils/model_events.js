 ...
 */
function withEvents(Model, name, options){
	const modelName = name || Model.name;
	const key = Model._key;
	// Which mutations announce themselves. A model with a hot, uninteresting
	// write path can narrow it — see ApiToken, whose last_used_on touch happens
	// on every authenticated API call.
	const actions = (options && options.actions) || ['create', 'update', 'remove'];

	const origCreate = Model.create;
	Model.create = async function(...args){
		const instance = await origCreate.apply(this, args);
		if (actions.includes('create')) emit(modelName, 'create', instance && instance[key], instance);
		return instance;
	};
	...
	Model.prototype.update = async function(...args){
		...
		if (actions.includes('update')) emit(modelName, 'update', this[key], result || this);
		return result;
	};
	...
	Model.prototype.remove = async function(...args){
		...
		if (actions.includes('remove')) emit(modelName, 'delete', pk, null);
		return result;
	};
