...
- **The notification bell can be filtered.** `@simpleworkjs/frontend` 0.4.3 →
  ...
  into; without it muting still works, it just has no UI.
...
- **The Font Awesome SVG-with-JS build is no longer loaded.**
  ...
  Found in theta-directory; the jump host loads the same shell, so it carried
  the same cost.
...
- **`app.notify` grew without bound in any long-lived tab.** It subscribes to
  ...
  4GB; the jump host's UI loads the same shell and the same feed, so it carried
  the same defect. Fixed upstream in `@simpleworkjs/frontend` v0.4.3.
- **`nodejs/package.json` said `3.7.0` on a tree released as v3.8.0.** The
  v3.8.0 release updated the changelog and was tagged, but never bumped the
  version field, so the package reported a stale version. Corrected here.
...
### Added
- **ID token signatures are now verified.** `@simpleworkjs/oidc-client` 1.0.0 → 1.1.1, which verifies the ID token against the SSO's JWKS (signature, `iss`, `aud`, `exp`) and cross-checks its subject against the userinfo response. Previously the flow was treated as opaque and identity came from userinfo alone — sound, but it established nothing about *who asserted* it. The client's own comment gave the reason: "The SSO publishes no jwks_uri". theta-directory v2.38.0 publishes one.

  Verification requires `oidc.jwksUri`, which `bootstrap/bootstrap.js` now writes pointing at the SSO over loopback (`http://127.0.0.1:3001/.well-known/jwks.json`) — the same reason `tokenEndpoint` and `userinfoEndpoint` already use it. `issuer` deliberately stays the public host: it is what the SSO puts in the token's `iss` claim, and that is what gets compared. An existing deployment without `jwksUri` keeps working exactly as before, logging one warning that signatures are not being verified.
...
- fix: **`isCatalogHost` now actually filters.** It excludes resources the SSO merely *discovered* and nobody promoted, by reading `managed` and `discovery_sources` — but neither key was declared in `@simpleworkjs/directory-schema`'s `METADATA_KEYS`, so `projectResource()` stripped both before they reached this app. And this app is a machine caller, which `isDirectoryAdmin()` never treats as an admin. Every resource therefore arrived with neither field, `autoDiscovered` computed `false`, and the filter returned `true` for everything it was asked about — including the unpromoted Proxmox guests and UniFi clients that v3.3.0 believed it had excluded.
  ...
  `accessibleHosts()` was unaffected in practice: the SSO applies the same catalog rule server-side before answering `/api/discovery/access/:uid`, so nothing unpromoted was in the list to begin with. **`allHosts()` — the unfiltered admin host view — was affected**, and is the visible change here: unpromoted discovery output no longer appears in it.

  Fixed in `@simpleworkjs/directory-schema` v1.2.0, which declares both keys as public. Requires ≥1.2.0; the dependency range says so now.

- chore: **the lockfile was pinning `directory-schema` at 1.0.0** despite a `^1.0.0` range, so this app had never received v1.1.0 either. The lockfile is what CI and production install, so the range alone was not enough.

## [3.3.9] - 2026-08-25
- docs: **a host running theta-agent is a jump target automatically.** Fixed in `theta-directory` v2.28.0 — the SSO's access projection now offers any agent-enrolled host to users whose groups already cover hosts at that site (`god_admin`, `{site}_super_admin`, or the `{site}_hosts_access` / `{site}_hosts_admin` aggregate), with no `<slug>_access` group to create first. Installing the agent needs root on the machine *and* a join key from the directory, so the machine is already under management. No change in this repo — `accessibleHosts()` already offers whatever `/api/discovery/access/:uid` returns — but the behaviour is this app's, so `docs/connecting.md` documents it here. Services an agent registered follow the same host and have no access groups of their own.
- docs: **recorded that `isCatalogHost` cannot see what it keys on.** `managed` and `discovery_sources` are not declared in `@simpleworkjs/directory-schema`'s `METADATA_KEYS`, so `projectResource()` strips both for any caller that is not a directory admin — and `isDirectoryAdmin()` is false for `isMachine`, which is what this app is. Every resource therefore arrives with neither field, `autoDiscovered` computes false, and the filter returns true for everything. `accessibleHosts()` is unaffected in practice, because the SSO applies the same catalog rule server-side before answering; `allHosts()` — the unfiltered admin view — is affected. Fixing it properly means declaring the two keys in `directory-schema`, which is outside this repo; the comment now says so rather than claiming a filter that does not filter.
